...
const CHART_LIMIT = clampInt(process.env.CHART_LIMIT, 25, 0, 300);
...
async function main(){
  ...
  for (const c of mappedCoins) {
    ...
    const detail = {
      ...
      name: c.name,
      slug: src?.slug ?? null,
      rank: src?.cmc_rank ?? null,
      price: q.price ?? null,
      marketCap: q.market_cap ?? null,
      volume24h: q.volume_24h ?? null,
      circulating: src?.circulating_supply ?? null,
      totalSupply: src?.total_supply ?? null,
      maxSupply: src?.max_supply ?? null,
      percentChange1h: q.percent_change_1h ?? null,
      percentChange24h: q.percent_change_24h ?? null,
      percentChange7d: q.percent_change_7d ?? null,
      lastUpdated: q.last_updated ?? null,
      logo: meta[c.id]?.logo ?? null,
      tags: meta[c.id]?.tags ?? [],
      category: meta[c.id]?.category ?? null,
      platform: meta[c.id]?.platform ?? null,
      selfReportedCirculatingSupply: meta[c.id]?.self_reported_circulating_supply ?? null,
      selfReportedMarketCap: meta[c.id]?.self_reported_market_cap ?? null,
      geckoId: c.geckoId
    };
  ...
  for (const coin of chartCoins) {
    ...
    for (const d of chartDaysList) {
      try {
        ...
        // Make sure frontend always has something to read.
        writeEmptyMarketChart(coin.id, d);
        const ohlcSaveDays = (d === 'max') ? '365' : d;
        writeEmptyOHLC(coin.id, ohlcSaveDays);
      }

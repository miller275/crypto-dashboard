...
async function fetchCurrencies(convert) {
  ...
  // CryptoRank v2 validates enums/limits strictly; we try a few safe variants.
  const limitCandidates = [100, 50, 25];
  const baseCandidates = [
    // Sorting + includes (different casings). If invalid, we'll fall back to minimal params.
    { sortBy: "marketCap", sortDirection: "DESC", include: "VALUES,IMAGES,LINKS" },
    { sortBy: "marketCap", sortDirection: "DESC", include: "values,images,links" },
    { sortBy: "marketCap", sortDirection: "desc", include: "values,images,links" },
    { sortBy: "marketCap", sortDirection: "DESC", include: "values" },
    { sortBy: "marketCap", sortDirection: "DESC" },
    { sortBy: "marketCap" },
    {}, // safest fallback
  ];
  ...
  // 1) detect a working query template using offset=0
  let template = null;
  let firstList = null;
  ...
  for (const limit of limitCandidates) {
    for (const base of baseCandidates) {
      for (const curStyle of currencyStyles) {
        const params = { limit, offset: 0, ...base, ...curStyle };
        try {
          const resp = await cr("/currencies", params);
          const list = extractList(resp);
          if (Array.isArray(list) && list.length) {
            template = { limit, ...base, ...curStyle };
            firstList = list;
            break detect;
          }
        } catch (e) {
  ...
  if (!template) throw lastErr || new Error("Failed to fetch /currencies");

  // 2) paginate best-effort to ~250 items (enough for the site table/pagination)
  const desired = 250;
  const limit = Number(template.limit) || 50;
  const all = (firstList || []).slice();

  for (let offset = limit; offset < desired; offset += limit) {
    try {
      const resp = await cr("/currencies", { ...template, offset });
      const list = extractList(resp);
      ...
      // If paging fails, keep what we already have (better than crashing the whole workflow).

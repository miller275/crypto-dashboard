...
async function fetchCurrencies(convert) {
  const baseParams = {
    ...
    sortBy: "marketCap",
    sortDirection: "desc",
    include: "values,images,links,description,tags",
  };

  const tries = [
    { ...baseParams, currency: convert },
    { ...baseParams, convert: convert },
    { ...baseParams, quote: convert },
    { ...baseParams }, // fallback (default currency)
  ];

  let lastErr = null;
  for (const params of tries) {
    try {
      return await cr("/currencies", params);
    } catch (e) {
      lastErr = e;
      await sleep(300);
    }
...
async function build(convert) {
  ...
  const listResp = await fetchCurrencies(convert);
  const unwrapped = unwrapData(listResp);
  const list = Array.isArray(unwrapped) ? unwrapped : (unwrapped && Array.isArray(unwrapped.data) ? unwrapped.data : (unwrapped && Array.isArray(unwrapped.items) ? unwrapped.items : []));

...
## Если на сайте пусто
Это нормально до первого обновления снапшотов.
Сделай так:
1) Repo → **Settings → Secrets and variables → Actions** → добавь секрет `CMC_PRO_API_KEY`
2) Repo → **Actions** → workflow **Update market data** → **Run workflow**
3) Дождись зелёного статуса и обнови страницу сайта

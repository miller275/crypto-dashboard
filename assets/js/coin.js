...
async function hydrate(id, {keepChart=false}={}){
  ...
  const desc = (c.description?.en ? c.description.en : "").trim();
  const descEl = Utils.qs("#coinDesc");
  if (desc){
    descEl.textContent = desc;
  }else{
    descEl.innerHTML = `<span class="muted">${I18N.get()==="ru" ? "Нет описания" : "No description"}</span>`;
  }

  Utils.qs("#aboutHint").textContent = (I18N.get()==="ru")
    ? "Источник описания: CoinMarketCap"
    : "Description source: CoinMarketCap";

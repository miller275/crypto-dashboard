...
export const API = {
  ...
  async loadMarketChart(id, days){
    ...
    // Be resilient: if chart snapshot is missing, return an empty dataset
    // so the coin page still renders (no external API calls from browser).
    if (!snap || !Array.isArray(snap.prices)) {
      return { source: "snapshot", data: { prices: [] }, updatedAt: null };
    }
    return { source: "snapshot", data: snap, updatedAt: snap.updatedAt || null };
  },

  async loadOHLC(id, days){
    const snap = await trySnapshot(PATHS.ohlc(id, days));
    // Same idea: missing OHLC should not break the UI.
    if (Array.isArray(snap)) return { source: "snapshot", data: snap, updatedAt: null };
    return { source: "snapshot", data: [], updatedAt: null };
  }
};

...
(function () {
  ...
  function requireData(snapshot) {
    ...
    if (!ok) {
      const msg =
        "Нет данных. Для CoinMarketCap Pro нужно обновить снапшоты через GitHub Actions (секрет CMC_PRO_API_KEY). " +
        "Открой README.md и запусти workflow 'Update market data'.";
      throw new Error(msg);
    }
  }

/* CryptoWatch — api.js (CryptoRank API via GitHub Pages snapshots + alternative.me F&G)
   IMPORTANT: CryptoRank API API нельзя дергать напрямую из браузера (CORS + утечка ключа).
...
(function () {
  ...
  async function getSnapshot(vs) {
    ...
    if (!ok) {
      ...
      return {
        source: (snap && snap.source) || "cryptorank",
